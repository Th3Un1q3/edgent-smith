...
- **CI Fast Path**: `just ci` runs 12 gates (~4–5 min incl ~4m10s mutation, timeout 720s). For `*.md`/`*.yml`/`justfile` docs-only changes use `just ci-fast` or `SKIP_MUTATION=1 just ci` (~20s); mutation only matters for `.opencode/**/*.ts`. Remote PR CI runs `just ci` verbatim — keep `scripts/ci.sh` and `.github/workflows/ci.yml` in sync. Env `MUTATION_TIMEOUT=720` wraps `timeout` gate 12; `MUTATION=1` opts into full run.
