...
9. **Write for the reader** — strip meta-commentary and author-process markers before finishing; author-process records live in frontmatter `metadata.delta` (Rule 14). Keep navigation: When to load lines, routing rows, cross-file pointers. Run the Rule 14 audit command from [guidance.md](../references/guidance.md) and `python3 scripts/audit_fences.py .`; fix every hit. Verify every fact and example against a trusted source; never trust parametric memory (Rule 23).
