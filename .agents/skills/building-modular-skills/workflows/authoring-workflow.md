...
- Read [SKILL.md](../SKILL.md) first — the 8 Principles, the Completion Gate, and the Task Routing Table define what the finished skill must contain.
...
8. **Wire rules to worked examples** — mark each numeric or behavioral rule with a copy-pasteable example labeled "Implements:". Link every file in the routing table with relative links such as `./workflows/create.md`. Reuse: a workflow or reference file can be linked from multiple root skills.
9. **Validate** — run the [Shaping Checklist](./shaping-checklist.md) as the completion gate; fix every failing check before you continue. Confirm every linked file loads independently without the root. Confirm the skill stays narrow and does not solve unrelated tasks.
...
- Root `SKILL.md` stays ≤ ~90 lines.
- Every file in the skill tree (workflows, references, recipes, scripts) appears in the routing table with a working relative link.
- Every principle starts with an active-verb kicker.
- Every numeric rule has an "Implements:" example.
- Zero broken links — resolve every cross-reference and grep the tree for stale naming.
- [Shaping Checklist](./shaping-checklist.md) passes 13/13.
