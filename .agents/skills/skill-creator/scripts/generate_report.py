...
"""
        + (
            ...
            "family=Poppins:wght@500;600&family=Lora:wght@400;500&"
...
"""
        + (
            ...
            "This page updates automatically as Claude tests different "
            "versions of your skill's description. Each row is an "
            "iteration \u2014 a new description attempt. The columns show "
            "test queries: green checkmarks mean the skill triggered "
            "correctly (or correctly didn't trigger), red crosses mean it "
            'got it wrong. The "Train" score shows performance on queries '
            'used to improve the description; the "Test" score shows '
            "performance on held-out queries the optimizer hasn't seen. "
            "When it's done, Claude will apply the best-performing "
...
"""
    ...
    html_parts.append(
        f"""\n    <div class="summary">\n"""
        f"""        <p><strong>Original:</strong> """
        f"""{html.escape(data.get("original_description", "N/A"))}</p>\n"""
        f"""        <p class="best"><strong>Best:</strong> """
        f"""{html.escape(data.get("best_description", "N/A"))}</p>\n"""
...
""")
        ...
        for qinfo in test_queries:
            ...
            html_parts.append(
                f'                <td class="result test-result {css_class}">{icon}'
                f'<span class="rate">{triggers}/{runs}</span></td>\n'
            )
