...
| `take_snapshot` | Text snapshot of the a11y tree with uids | ONLY permitted use: ONE truncated snapshot (~2 KB / text-only) for structure discovery on unfamiliar sites (workflow Step 4); never for extraction; snapshot uids are ephemeral, never persist or reuse them |
...
10. Output/token discipline: snapshots and full-DOM dumps flood context; return only needed fields with a hard 3 KB cap per result — oversized results are re-run trimmed, never passed through.
11. `evaluate_script` returns a **markdown-wrapped string, not bare JSON** — the response is `Script ran on page and returned:` followed by a `json`-fenced block containing the value. A naive `JSON.parse` on the whole string fails, loses the data, and forces full re-extraction. Strip to the first `{` and the last `}` before parsing.
...
15. `take_snapshot` returns the FULL a11y tree — no truncation parameter exists; it can be tens of KB. The script must truncate/filter before returning. An untruncated snapshot is a context blowout.
...
| `take_snapshot` | a11y text snapshot with uids | ONLY permitted use: ONE truncated snapshot (~2 KB) for structure discovery on unfamiliar sites; never for extraction; click by text instead of uid-based clicks |
