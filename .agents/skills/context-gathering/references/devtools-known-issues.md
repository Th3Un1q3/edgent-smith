...
19. **`filePath` on file-writing tools is DENIED — the server runs on the HOST, not in the agent's container.** `take_snapshot`, `evaluate_script`, and `take_screenshot` all refuse a `filePath` with `Error: Access denied: path <p> (canonical: <host-canonical>) is not within any of the configured workspace roots` (verified live 2026-08-13 for `/tmp/x`, `/workspace/x`, and relative `.tmp/x`). The server is launched by `.devcontainer/init.sh` on the host OS, so canonical paths look like host paths (`/private/tmp/...`, `/Users/<user>/...`) and any file it writes never lands where the container agent can read it. Only INLINE returns work: `take_snapshot` without `filePath` (truncate in-script per §A), `evaluate_script` without `filePath`. Never invent a path for devtools `filePath` arguments — treat every such write as failing.
20. **Gateway attach is transiently flaky.** "Not connected" or an empty tool list on activation happens; retry once or twice before concluding failure. Sustained failure after retries means the host proxy or Chrome is down — escalate to the operator (`.devcontainer/init.sh` restarts them); never fall back to fetch/tavily/search APIs/web archives, which change the source of truth ([workflows/browser-automation-devtools.md](../workflows/browser-automation-devtools.md) — "Source of truth down — ESCALATE, don't workaround").
21. **The code-mode JS harness mangles `\"` and `\\` inside nested strings.** Write scripts with zero backslashes and zero double quotes; build special characters via `String.fromCharCode` (workflow Shared rules — zero-backslash rule).
22. **Navigate in one call, evaluate in the next.** `new_page`/`navigate_page` followed by a heavy `evaluate_script` in the SAME call times out; split navigation (D1) and evaluation (D2 / next call) across calls.
23. **`evaluate_script` runs in the browser realm, not Node.js.** No CommonJS modules and no Node.js runtime globals — `require`, `module`, `process`, `Buffer`, `fs` are all undefined. Calling `require()` throws `ReferenceError: require is not defined`; Node-style source the browser parser rejects throws `Invalid or unexpected token` (observed session 2026-08-16). Write plain browser JS — DOM and Web APIs only (`document`, `window`, `fetch`). Reuse the battle-tested snippets and page helpers in [references/snippets.md](../references/snippets.md) (X, A–E, B2, D2) instead of re-inventing Node-style scripts.
