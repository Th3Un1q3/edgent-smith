...
One gate governs every memory write.
...
- [ ] **Right-size** — at the right abstraction level: no line numbers, exact paths, or code dumps that will stale; ≤ 3 paragraphs. *If not:* over-specific detail misleads after the code changes → raise the abstraction level.
  ...
  - **Exemption — extraction memories**: `browser-automation/<site>/<task>-extraction` memories exist to hold a selector list + URL template + verification date — operating data by design — so they are exempt from the "no code/selector dumps" clause, as are dated result caches (exemption above). The exemption covers content shape only; it does NOT waive the other gate checks (Verified — name the source; Discoverable; Privacy; etc.).
...
- If the gate changes, update the pointer in the store recipe.
