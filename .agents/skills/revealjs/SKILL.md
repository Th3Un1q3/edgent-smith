...
metadata:
    github-path: skills/revealjs
    github-ref: refs/heads/main
    github-repo: https://github.com/ryanbbrown/revealjs-skill
    github-tree-sha: 17fd2dcca417d9c931f88b45bf1b43a389c7fa87
name: revealjs
---
...
Every deck-edit prompt and every validator pass over a reveal.js deck MUST follow the [references/deck-authoring-invariants.md](references/deck-authoring-invariants.md). Highlights: never put box-affecting styles (`border`/`padding`/`margin`) on a slide `<section>` — use an inner wrapper `<div>`; a chapter stack container holds ONLY nested `<section>`s; `<aside class="notes">` is the last child of a slide `<section>`; Mermaid diagrams are raw `<div class="mermaid"><pre>` blocks. Validators MUST FAIL any box-affecting inline style on a `<section>`.

Deck build conventions — metacommentary placement, placeholder text, fragments, escaping, structural labels, and preserving external edits — are in [references/deck-conventions.md](references/deck-conventions.md).

## Post-Edit Completion Gate

Before reporting a deck edit complete, run this gate and report PASS/FAIL evidence per item in the final message:

1. **Tag balance**: verify opening/closing counts match for `<section>`, `<aside>`, `<table>`, `<tr>`, `<th>`, `<td>`, `<div>`. When the prompt supplies expected counts (e.g. "77/77 sections"), verify against them.
2. **Href audit**: independently count the nested-slide sequence of the edited chapter and verify EVERY fragment link (e.g. `#/4/N`) resolves to the intended slide. Any reorder/add/remove invalidates index-based links — recount and re-point all affected hrefs.
3. **Convention greps** (report pass/fail per item with counts):
   - no trailing periods: `.</p>` / `.</td>` / `.</h2>` / `.</h3>` occurrences = 0
   - no `<!-- TODO` HTML comments (TODO text inside `<aside class="notes">` is allowed)
   - `<aside class="notes">` is the last child of every slide `<section>`
   - `&amp;` escaping for visible `&` characters
   - plain `<section>`s: no attributes on section tags
   - stack-container sections contain only nested sections (no direct text)
4. Report the gate results as explicit PASS/FAIL evidence in the final message.

## Slide Layout Rules

1. Never apply box-affecting styles (border, padding, margin, width, height — inline or class) directly on a reveal.js slide `<section>` element. reveal.js measures the section element to fit the viewport; box styles on it overflow the screen. Wrap styled content in an inner `<div>` (e.g. a frame/border div) instead.
2. Vertical-stack containers (a `<section>` containing nested `<section>`s) must contain ZERO direct text/content — only nested sections. Text directly in a stack container persists across nested slides and overlaps them.
3. Deck validation: check that no `<section>` carries attributes or inline styles.
