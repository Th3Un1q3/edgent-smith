...
4. **Structure** — `loop-data`/`loop-elements` loops need a `loop-breakpoint` with a matching `loopId` at the END of the loop body; `repeat-task` loops must NOT be paired with one — they iterate via continuation edges back into the repeat-task node's input-1 and exit via output-1, and a breakpoint wired to a repeat-task loopId crashes the workflow (verified against AutomaApp/automa engine source: handlerRepeatTask.js, handlerLoopBreakpoint.js) ([design-patterns.md](../references/design-patterns.md#give-every-loop-dataloop-elements-loop-a-loop-breakpoint-with-the-same-loop-id-repeat-task-takes-none)); every conditions block needs a fallback path ([design-patterns.md](../references/design-patterns.md#conditions-route-to-a-matching-condition-output-or-the-fallback)).
...
7. **Loops pair with breakpoints.** Every `loop-data` or `loop-elements` node has a `loop-breakpoint` node with the same `loopId` at the END of the loop body; a `repeat-task` node must have NO `loop-breakpoint` — it iterates via continuation edges back into its input-1 (exit via output-1) and never populates the engine's loop list, so a breakpoint wired to a repeat-task loopId throws `Can't find a loop with "<loopId>" loop id` on the first iteration (verified against AutomaApp/automa engine source: handlerRepeatTask.js, handlerLoopBreakpoint.js) ([design-patterns.md](../references/design-patterns.md#give-every-loop-dataloop-elements-loop-a-loop-breakpoint-with-the-same-loop-id-repeat-task-takes-none)). No loop nodes → check passes.
...
- **Selectors** — check against the LIVE page before relying on them. Classes may be hashed (A/B render); prefer stable attributes (`data-testid`, `aria-label`, `componentkey`, `role`, text markers). On list pages, scope the card selector to the results container — e.g. `//div[contains(@class, '<results-container>')]//div[...card...]` — so recommended/similar sections do not match. English-literal text selectors (`'Next'`, `'About the job'`, `'Save'`) are locale-fragile. Third-party JS-rendering extractors return normalized markdown that lies about the DOM — headings may be styled `div`s, 'Show more' buttons absent, classes build-hashed (`*-sc-*`); confirm every selector against the live browser and, when one is unavailable, say so — never ship guessed selectors as fact. When the page embeds JSON-LD, parse it before walking the DOM ([design-patterns.md §Prefer embedded structured data](../references/design-patterns.md#prefer-embedded-structured-data-json-ld-over-dom-traversal)).
...
- [ ] Every `loop-data`/`loop-elements` node pairs with a `loop-breakpoint` carrying the same `loopId`; no `repeat-task` node is paired with a `loop-breakpoint`.
