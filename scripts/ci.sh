...
# ~20s local iteration.
# Timeout: opencode-mutation is wrapped with `timeout ${MUTATION_TIMEOUT:-1260}` when GNU timeout is
# available (default 1260s, env MUTATION_TIMEOUT overrides); workflow-level timeouts are 12/30 min.
set -euo pipefail
...
if [[ "${MUTATION:-0}" == "1" ]]; then
  _ci_mutation_timeout="${MUTATION_TIMEOUT:-1260}"
