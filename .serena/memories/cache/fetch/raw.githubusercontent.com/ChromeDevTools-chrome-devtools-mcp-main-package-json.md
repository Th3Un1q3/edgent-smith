tool: fetch
url: https://raw.githubusercontent.com/ChromeDevTools/chrome-devtools-mcp/main/package.json
date: 2026-08-06
source: fetch

Content type text/plain; charset=utf-8 cannot be simplified to markdown, but here is the raw content:
Contents of https://raw.githubusercontent.com/ChromeDevTools/chrome-devtools-mcp/main/package.json:
{
  ...
  "scripts": {
    ...
    "test": "npm run build && node scripts/test.mjs --test-skip-pattern=THIRD_PARTY_NOTICES",
    "test:no-build": "node scripts/test.mjs --test-skip-pattern=THIRD_PARTY_NOTICES",
    "test:notices": "npm run bundle && node scripts/test.mjs tests/third_party_notices.test.ts",
    "test:notices:no-build": "node scripts/test.mjs tests/third_party_notices.test.ts",
